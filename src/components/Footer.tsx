import React, { useState } from 'react'; // Tambahkan React.FormEvent di sini jika menggunakan TSX
 ...
 */
// Jika menggunakan TSX, tanda (e) bisa diatasi dengan (e: React.FormEvent<HTMLFormElement>)
const NewsletterForm = () => { 
    ...
    // Endpoint API yang DITARGETKAN: /api/applications
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Data yang dikirim HARUS mencakup semua bidang di skema Application MongoDB 
            // agar tidak terjadi error validasi, meskipun hanya email yang diisi.
            const payload = {
                email: email,
                nama: `[NEWSLETTER] ${email}`, // Label khusus di DB
                whatsapp: '',
                sekolah: 'Newsletter Subscription',
                jurusan: '',
                posisi: 'Newsletter Subscriber', // Label posisi khusus
                link_cv: '',
                motivasi: 'Hanya ingin mendapatkan update via email.',
            };
            ...
            if (response.ok) {
                ...
                setIsError(false);
                setMessage("Berhasil berlangganan! Data Anda ('Newsletter Subscriber') telah dicatat.");
                setEmail(''); // Kosongkan input setelah sukses
            } else if (response.status === 409) {
                // Skenario Konflik (Email sudah terdaftar) - Sesuai logika server.js (Error 11000)
                setIsError(true);
                setMessage(data.message || "Email ini sudah terdaftar. Tidak perlu subscribe lagi.");
            } else {
                // Skenario Gagal Lainnya (Status 400 Bad Request / 500 Internal Server Error)
                setIsError(true);
                setMessage(data.message || "Gagal berlangganan. Silakan periksa email Anda atau coba lagi.");
            }
            ...
            setIsError(true);
            setMessage("Koneksi gagal. Pastikan Server Node.js Anda berjalan di http://localhost:5000.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-4">
            ...
            {message && (
                <div 
                    className={`p-3 rounded-xl text-sm font-medium transition-all duration-300 ${isError ? 'bg-red-700 border border-red-500 shadow-xl' : 'bg-green-700 border border-green-500 shadow-xl'}`}
                    role="alert"
                >
                    {message}
                </div>
...
export default function App() {
  ...
  // URL untuk Policy dan Terms (Anda bisa mengganti URL ini sesuai kebutuhan)
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10 mb-8">
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li>
                <a href="#about" className="text-blue-300 hover:text-orange-400 transition-all text-sm block transform hover:translate-x-1">
                  Tentang Program
                </a>
              </li>
              <li>
                <a href="#positions" className="text-blue-300 hover:text-orange-400 transition-all text-sm block transform hover:translate-x-1">
                  Posisi Tersedia
                </a>
              </li>
              <li>
                <a href="#gallery" className="text-blue-300 hover:text-orange-400 transition-all text-sm block transform hover:translate-x-1">
                  Galeri
                </a>
              </li>
              <li>
                <a href="#testimonials" className="text-blue-300 hover:text-orange-400 transition-all text-sm block transform hover:translate-x-1">
                  Testimonial
                </a>
              </li>
              <li>
                <a href="#faq" className="text-blue-300 hover:text-orange-400 transition-all text-sm block transform hover:translate-x-1">
                  FAQ
                </a>
              </li>
            </ul>
          ...
          {/* Kolom 4: Newsletter - Menggunakan komponen NewsletterForm yang berfungsi */}
          <div>
            <NewsletterForm />
          </div>
