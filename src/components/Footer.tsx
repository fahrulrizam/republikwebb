...
const NewsletterForm = () => { 
    ...
    // PERBAIKAN KRITIS TERBARU: URL Disesuaikan menjadi /api/register
    // Sesuai dengan rute yang didefinisikan di server.js Anda.
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            const payload = {
                ...
                // --- PERBAIKAN ENUM VALIDASI MONGODB ---
                // Nilai 'Newsletter Subscriber' kemungkinan tidak ada di daftar enum.
                // Menggantinya dengan 'Lainnya' (asumsi ini adalah opsi umum di enum Anda).
            ...
            // Mencoba mendapatkan body respons sebagai JSON. 
            let data = {};
            try {
                // Cek dulu apakah ada content-type JSON
                const contentType = response.headers.get("content-type");
                if (contentType && contentType.indexOf("application/json") !== -1) {
                    data = await response.json();
                } else {
                    // Jika bukan JSON, respons mungkin HTML (404)
                    console.error(`Respons server bukan JSON (Status: ${response.status}). Kemungkinan 404 page atau error server.`);
                    data = { error: `Server merespons dengan status ${response.status}. Body respons bukan JSON.` };
                }
            } catch (jsonError) {
                // Jika gagal parse, ini menangani SyntaxError: Unexpected token '<'
                console.error("Gagal parse respons sebagai JSON.", jsonError);
                data = { error: "Terjadi kesalahan saat memproses respons server." };
            }
            
            if (response.ok || response.status === 201) {
                // Skenario Sukses (Status 200 OK atau 201 Created)
                setIsError(false);
                setMessage("🎉 Berhasil berlangganan! Data Anda telah dicatat di MongoDB.");
                ...
                setMessage(data.error || "Email ini sudah terdaftar. Tidak perlu subscribe lagi.");
                 ...
                 // Penanganan 404 secara eksplisit
                setIsError(true);
                // Pesan yang sangat spesifik
                setMessage(`ERROR 404: Rute API "${API_URL}" tidak ditemukan di server. Pastikan server berjalan dan rute di server.js adalah '/api/register'.`);
